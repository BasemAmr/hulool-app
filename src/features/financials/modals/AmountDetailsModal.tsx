  ...
  return (
    <BaseModal isOpen={true} onClose={closeModal} title="تفاصيل المبلغ" className="amount-details-modal">
      <div className="max-h-screen overflow-y-auto space-y-4">
        {/* Header Info */}
        <div className="bg-bg-surface-muted rounded-lg p-4 border border-border-strong space-y-3">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <h6 className="font-semibold text-text-primary flex items-center gap-2 m-0">
              <Layers size={18} className="text-status-warning-text" />
              بنود المبلغ
            </h6>
            <Button
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {amountDetailsArr.map((detail, index) => (
              <div 
                key={index} 
                className="flex justify-between items-start p-3 rounded-lg border border-border-default bg-bg-surface-muted transition-all duration-200 hover:shadow-md hover:-translate-y-0.5"
              >
                <div className="flex-1">
                  <div className="font-medium text-text-primary mb-1">{detail.description}</div>
        ...
        {/* Footer Actions */}
        <div className="flex justify-end gap-2 pt-4 border-t border-border-strong">
