    ...
    <div className="p-6">
      ...
      <div className="flex flex-wrap justify-between items-center gap-2 mb-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold text-text-primary">حساب الشركة</h1>
        </div>
        
        {/* Inline Summary Stats */}
        <div className="flex items-center gap-3 text-xs">
          <div className="flex items-center gap-1">
            <span className="text-text-secondary">الرصيد الحالي:</span>
            <span className={`font-bold ${
              currentBalance < 0 ? 'text-status-danger-text' :
              currentBalance > 0 ? 'text-status-success-text' : 'text-text-secondary'
            }`}>
              {formatCurrency(currentBalance)} ر.س
            </span>
            {currentBalance < 0 && <span className="text-status-danger-text">(التزامات)</span>}
          </div>
          <div className="w-px h-4 bg-gray-300" />
          <div className="flex items-center gap-1">
            <span className="text-text-secondary">إجمالي المدين:</span>
            <span className="font-bold text-status-success-text">{formatCurrency(summary.totalDebits)} ر.س</span>
          </div>
          <div className="w-px h-4 bg-gray-300" />
          <div className="flex items-center gap-1">
            <span className="text-text-secondary">إجمالي الدائن:</span>
            <span className="font-bold text-status-info-text">{formatCurrency(summary.totalCredits)} ر.س</span>
          </div>
          <div className="w-px h-4 bg-gray-300" />
          <div className="flex items-center gap-1">
            <span className="text-text-secondary">عدد المعاملات:</span>
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-12 text-text-muted">لا توجد معاملات</div>
          ) : (
            <div className="overflow-x-auto">
              <Table className="border-collapse border border-border-strong">
                <TableHeader>
                  <TableRow>
                    <TableHead className="text-start border border-border-strong text-base px-3 py-2">التاريخ</TableHead>
                    <TableHead className="text-start border border-border-strong text-base px-3 py-2">النوع</TableHead>
                    <TableHead className="text-start border border-border-strong text-base px-3 py-2">الوصف</TableHead>
                    <TableHead className="text-start border border-border-strong text-base px-3 py-2">مدين</TableHead>
                    <TableHead className="text-start border border-border-strong text-base px-3 py-2">دائن</TableHead>
                    <TableHead className="text-start border border-border-strong text-base px-3 py-2">الرصيد</TableHead>
                    <TableHead className="text-start border border-border-strong text-base px-3 py-2">القيد المقابل</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                    ...
                    <TableRow key={txn.id}>
                      ...
                      <TableCell className="border border-border-strong text-start text-base px-3 py-2">
                        {txn.credit && parseFloat(txn.credit) > 0 ? (
                          <span className="text-status-info-text font-semibold">
