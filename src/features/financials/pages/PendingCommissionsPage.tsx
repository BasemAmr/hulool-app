...
import EditExpectedCommissionModal from '@/features/employees/modals/EditExpectedCommissionModal';
      ...
      {allItems.length === 0 ? (
        ...
        <div className="rounded-lg border border-border bg-card shadow-sm overflow-hidden">
          <Table className="border-collapse border border-border-strong">
            <TableHeader>
              <TableRow>
                <TableHead className="text-right border border-border-strong text-base px-3 py-2">المهمة</TableHead>
                <TableHead className="text-right border border-border-strong text-base px-3 py-2">العميل</TableHead>
                <TableHead className="text-right border border-border-strong text-base px-3 py-2">مبلغ الفاتورة</TableHead>
                <TableHead className="text-right border border-border-strong text-base px-3 py-2">حالة الفاتورة</TableHead>
                <TableHead className="text-right border border-border-strong text-base px-3 py-2">العمولة المتوقعة</TableHead>
                <TableHead className="text-right border border-border-strong text-base px-3 py-2">الأيام المعلقة</TableHead>
                <TableHead className="text-right border border-border-strong text-base px-3 py-2">الإجراءات</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {allItems.map((item) => (
                <TableRow key={item.id}>
                  {/* Task Name */}
                  <TableCell className="border border-border-strong text-right text-base px-3 py-2 font-medium">
                    {item.task?.task_name || '-'}
                  </TableCell>

                  {/* Client Name */}
                  <TableCell className="border border-border-strong text-right text-base px-3 py-2">
                    <div>
                      <div className="font-medium">{item.client?.name || '-'}</div>
                      {item.client?.phone && (
                        <div className="text-base text-text-primary/50">
                          {item.client.phone}
                        </div>
                      )}
                    </div>
