...
import DashboardTaskTypeFilter from '../components/DashboardTaskTypeFilter';
import EmployeeTasksColumn from '../components/EmployeeTasksColumn';
...
const DashboardPage = () => {
    ...
    const taskTypeConfig = {
        ...
        Other: {
            ...
            displayName: 'أخرى',
            accentColor: 'var(--token-text-muted)',
        }
    };
                            ...
                            >
                                {employeeTasksGrouped.map((employeeGroup, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="h-full shadow-sm rounded-none overflow-visible relative flex flex-col transition-colors duration-150 w-full border border-border-default">
                                                {/* Header — white background, single left-border accent */}
                                                <div className="flex items-center gap-2 px-3 py-2 border-b border-border-default border-l-4 border-l-primary bg-bg-surface flex-shrink-0">
                                                    <Link
                                                        to={`/employees/${employeeGroup.employee_id}`}
                                                        className="no-underline flex-1 min-w-0"
                                                        title="عرض ملف الموظف"
                                                    >
                                                        <h6 className="mb-0 font-bold text-text-primary truncate hover:text-text-brand transition-colors">
                                                            المهام — {employeeGroup.employee_name}
                                                        </h6>
                                                    </Link>
                                                    <span className="flex-shrink-0 bg-bg-surface-muted border border-border-default rounded-full px-2 py-0.5 font-semibold text-xs text-text-secondary">
                                                        {totalClients}
                                                    </span>
                                                </div>

                                                {/* Body - Scrollable */}
                                                <div className="p-0 overflow-visible bg-transparent z-0 min-h-0 flex flex-col">
                                                    <EmployeeTasksColumn
