...
import { useDeferTask, useResumeTask, useRestoreTask, useRejectTask } from '@/features/tasks/api/taskQueries';
  ...
  return (
    <span className="hulool-cell-content" style={{ fontWeight: 600 }}>
      <Link
        ...
        className="no-underline text-text-primary font-bold hover:text-primary transition-colors"
...
// Type Badge Cell — all types use soft semantic token backgrounds
const TYPE_TOKEN_MAP: Record<string, { bg: string; text: string; border: string }> = {
  Government: {
    bg: 'var(--token-status-info-bg)',
    text: 'var(--token-status-info-text)',
    border: 'var(--token-status-info-border)',
  },
  RealEstate: {
    bg: 'var(--token-status-success-bg)',
    text: 'var(--token-status-success-text)',
    border: 'var(--token-status-success-border)',
  },
  Accounting: {
    bg: 'var(--token-status-warning-bg)',
    text: 'var(--token-status-warning-text)',
    border: 'var(--token-status-warning-border)',
  },
  Other: {
    bg: 'var(--token-status-neutral-bg)',
    text: 'var(--token-status-neutral-text)',
    border: 'var(--token-status-neutral-border)',
  },
};

const TypeBadgeCell = React.memo(({ rowData }: CellProps<Task, { t: (key: string) => string }>) => {
  const tokens = TYPE_TOKEN_MAP[rowData.type] || TYPE_TOKEN_MAP.Other;
...
// Status Badge Cell — maps every status to a semantic token set
const STATUS_TOKEN_MAP: Record<string, { bg: string; text: string; border: string }> = {
  New: {
    bg: 'var(--token-status-warning-bg)',
    text: 'var(--token-status-warning-text)',
    border: 'var(--token-status-warning-border)',
...
const StatusBadgeCell = React.memo(({ rowData, columnData }: CellProps<Task, { t: (key: string) => string }>) => {
  const t = columnData?.t || ((key: string) => key);
  const tokens = STATUS_TOKEN_MAP[rowData.status] || STATUS_TOKEN_MAP.Cancelled;
...
const ActionsCell = React.memo(({ rowData, rowIndex, columnData }: CellProps<Task, TaskActionsCellData>) => {
  ...
  // Status-based actions — all use neutral ghost style with semantic icon color only
  if (task.status === 'New') {
    actions.push({
      type: 'defer',
      onClick: () => handleDefer?.(task),
      title: 'تأجيل',
      icon: <Pause size={14} style={{ color: 'var(--token-status-warning-text)' }} />,
    });
  }
  ...
  if (task.status === 'Deferred') {
    actions.push({
      ...
      title: 'استئناف',
      icon: <Play size={14} style={{ color: 'var(--token-status-info-text)' }} />,
    });
  }
  ...
  if (task.status !== 'Completed' && task.status !== 'Pending Review' && shouldShowCompleteButton?.(task)) {
    actions.push({
      ...
      title: 'إكمال',
      icon: <CheckCircle size={14} style={{ color: 'var(--token-status-success-text)' }} />,
    });
  }
  ...
  if (task.assigned_to_id &&
    ...
    actions.push({
      ...
      title: 'مراجعة',
      icon: <ClipboardCheck size={14} style={{ color: 'var(--token-status-warning-text)' }} />,
    });
  }
  ...
  if (task.status === 'Completed' || task.status === 'Cancelled') {
    actions.push({
      ...
      title: 'استعادة',
      icon: <RotateCcw size={14} style={{ color: 'var(--token-status-info-text)' }} />,
    });
  }
