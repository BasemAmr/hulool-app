...
import { useResolvePrepaidChange } from '@/features/tasks/api/taskQueries';
...
const PrepaidConflictModal: React.FC<PrepaidConflictModalProps> = ({
        ...
        <div className="space-y-1">
          {conflictData.conflicts.map((conflict, index) => (
            <div key={index} className={`text-sm p-2 rounded ${
              ...
              conflict.severity === 'medium' ? 'border border-status-warning-border bg-status-warning-bg text-status-warning-text' :
              'border border-blue-600 bg-status-info-bg text-blue-800'
            }`}>
  ...
  const renderPaymentDecisions = () => (
    <div className="space-y-3">
      <h3 className="text-lg font-semibold text-text-primary">{t('tasks.paymentDecisions')}</h3>
      {conflictData.financial_records.payments.map((payment) => (
        <div key={payment.id} className="rounded-lg border border-border bg-card p-4 space-y-3">
          ...
          <div className="space-y-2">
            <label className="block font-semibold text-sm text-text-primary">{t('tasks.chooseAction')}:</label>
            <div className="space-y-2">
  ...
  const renderAllocationDecisions = () => (
    <div className="space-y-3">
      <h3 className="text-lg font-semibold text-text-primary">{t('tasks.allocationDecisions')}</h3>
  ...
  return (
    ...
    >
      <div className="space-y-4 max-h-96 overflow-y-auto">
        ...
        <div className="space-y-3">
          <h3 className="text-lg font-semibold text-text-primary">{t('tasks.prepaidReceivableDecision')}</h3>
          <select 
