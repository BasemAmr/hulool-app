  ...
  if (!canManageUsers) {
    return (
      <div className="text-center p-12 bg-card border border-border rounded-lg">
        <Shield className="text-text-primary mx-auto mb-4" size={48} />
        <h3 className="text-foreground mb-2 text-lg font-semibold">{t('users.noPermission') || 'لا تملك صلاحية إدارة المستخدمين'}</h3>
        <p className="text-text-primary">{t('users.contactAdmin') || 'اتصل بالمدير للحصول على الصلاحيات المطلوبة'}</p>
      </div>
    );
  }
  ...
  if (usersLoading) {
    return (
      <div className="text-center p-8">
        ...
        <p className="text-text-primary">{t('users.loading') || 'جاري تحميل المستخدمين...'}</p>
      </div>
    );
  }
