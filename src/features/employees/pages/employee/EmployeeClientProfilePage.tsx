...
import { useGetMyReceivablesDashboardInfinite } from '@/features/employees/api/employeeFinancialQueries';
        ...
        <div>
            ...
            {(mode === 'general' || mode === 'receivables') && (
                hasViewAllReceivablesPermission ? (
                    ...
                    <div className="rounded-lg border border-status-warning-border bg-status-warning-bg p-4 text-center">
                        <h5 className="text-status-warning-text font-bold">Access Denied</h5>
                        <p className="text-status-warning-text">You don't have permission to view receivables for this client.</p>
                    </div>
                )
            )}
