...
import { GridActionBar, createEditAction, createAddTaskAction, createAddInvoiceAction } from '@/shared/grid';
import type { GridAction } from '@/shared/grid';
import { useModalStore } from '@/shared/stores/modalStore';
import { useGetEmployeeClients } from '@/features/employees/api/employeeQueries';
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      <button
        ...
        onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'var(--token-status-success-bg)'}
...
const RegionCell = React.memo(({ rowData }: CellProps<EmployeeClient>) => {
  const getRegionColor = (regionName: string | null | undefined): string => {
    if (!regionName) return 'var(--token-status-neutral-bg)';
    const colors = [
      'var(--primitive-brand-600)',
      'var(--primitive-green-600)',
      'var(--primitive-blue-700)',
      'var(--primitive-amber-600)',
      'var(--primitive-red-600)',
...
const CreditCell = React.memo(({ rowData }: CellProps<EmployeeClient>) => {
  ...
  return (
    <div style={{ textAlign: 'center', fontWeight: 600, color: 'var(--token-status-success-text)' }}>
      {amount.toFixed(2)}
    </div>
  );
});
...
const EmployeeClientsTable: React.FC<EmployeeClientsTableProps> = ({
  ...
  if (clients.length === 0 && !isLoading) {
    return <div className="p-4 text-center text-text-primary">لا يوجد عملاء لهذا الموظف.</div>;
  }
