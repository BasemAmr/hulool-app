...
const AdminEmployeeInvoicesPanel: React.FC<AdminEmployeeInvoicesPanelProps> = ({ employeeId }) => {
  ...
  return (
    <div className="border border-border bg-card shadow-sm h-full flex flex-col overflow-hidden">
      {/* Header */}
      <div className="px-4 py-2 border-b border-border-default border-l-4 border-l-[color:var(--token-status-warning-text)] bg-bg-surface flex-shrink-0">
        <div className="flex justify-between items-center">
          <h6 className="mb-0 font-bold text-base text-text-primary">فواتير الموظف</h6>
          <span className="text-sm font-semibold text-text-secondary">{invoices.length} فاتورة</span>
        </div>
      </div>
        ...
        {invoices.length === 0 ? (
          <div className="flex justify-center items-center h-full">
            <div className="text-center">
              <p className="text-text-primary mb-0 text-base font-bold">
            ...
            <table className="w-full text-base border-collapse">
              <thead className="sticky top-0 bg-background z-10">
                <tr>
                  ...
                  <th className="text-sm px-1.5 py-1.5 border border-border-default text-center font-bold">المبلغ</th>
                  <th className="text-sm px-1.5 py-1.5 border border-border-default text-center font-bold">المدفوع</th>
                  <th className="text-sm px-1.5 py-1.5 border border-border-default text-center font-bold">المتبقي</th>
                  <th className="text-sm px-1.5 py-1.5 border border-border-default text-center font-bold">إجراءات</th>
                </tr>
              </thead>
              <tbody>
                {invoices.map((invoice, index) => {
                  ...
                  return (
                    <tr key={invoice.id}>
                      <td className={`text-base px-1.5 py-1 border border-border-default text-start font-bold ${bgColor}`}>
                        {clientName}
                      </td>
                      <td className={`text-base px-1.5 py-1 border border-border-default text-start font-bold ${bgColor}`}>
                        {invoice.description || invoice.task_name || `فاتورة #${invoice.id}`}
                      </td>
                      <td className={`text-base px-1.5 py-1 border border-border-default text-center font-bold ${bgColor}`}>
                        {formatCurrency(invoice.amount)}
                      </td>
                      <td className={`text-base px-1.5 py-1 border border-border-default text-center font-bold ${bgColor} ${invoice.paid_amount > 0 ? 'text-status-success-text' : 'text-text-secondary'}`}>
                        {formatCurrency(invoice.paid_amount)}
                      </td>
                      <td className={`text-base px-1.5 py-1 border border-border-default text-center font-bold ${bgColor} ${remaining > 0 ? 'text-status-danger-text' : 'text-status-success-text'}`}>
