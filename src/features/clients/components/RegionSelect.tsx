...
import { useGetRegions, useCreateRegion } from '@/features/clients/api/regionQueries';
  ...
  const renderDropdownContent = (onControllerChange?: (value: any) => void, currentValue?: number | null) => {
    ...
    return (
      ...
      {!required && (
        <button
          type="button"
          className="w-full px-3 py-2 text-right text-text-primary hover:bg-accent transition-colors duration-150 flex items-center gap-2"
      ...
      {/* Loading State */}
      {isLoading && (
        <div className="px-3 py-2 text-center text-text-primary">
        ...
        <>
          ...
          {!isCreating ? (
            ...
            <div className="p-3 bg-muted/50">
              <div className="mb-2">
                <label className="block text-sm text-text-primary mb-1">اسم المنطقة الجديدة:</label>
                <input
                  ref={createInputRef}
                  type="text"
                  className="base-input text-sm"
          ...
          render={({ field: { onChange: onControllerChange, value: controllerValue } }) => {
            ...
            return (
              <div className="relative" ref={dropdownRef}>
                ...
                >
                  <span className={selectedRegion ? '' : 'text-text-primary'}>
                    {selectedRegion ? selectedRegion.name : (placeholder || 'اختر منطقة')}
                  </span>
                  <i className={`fas fa-chevron-${isOpen ? 'up' : 'down'} text-text-primary`}></i>
                </button>
