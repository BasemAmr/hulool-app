...
import { useUpdateClientCredit, useResolveCreditReduction } from '@/features/clients/api/clientCreditQueries';
    ...
    <BaseModal isOpen={true} onClose={closeModal} title={t('credits.editCredit')}>
      <form onSubmit={handleSubmit(handleUpdate)} className="space-y-4">
        <div className="space-y-2">
          <label className="font-semibold text-text-primary text-sm block">{t('credits.currentAmount')}</label>
        ...
        {conflictData && (
          <div className="rounded-lg border border-status-warning-border bg-status-warning-bg p-4 space-y-3">
            <div>
              <h6 className="font-semibold text-text-primary">{t('credits.creditReductionConflict')}</h6>
              <p className="text-sm text-status-warning-text">{t('credits.cannotReduceBelowAllocated')}</p>
              <p className="text-sm text-status-warning-text"><strong>{t('credits.allocatedAmount')}: </strong><SaudiRiyalIcon amount={conflictData.allocated_amount} /></p>
              <p className="text-sm text-status-warning-text"><strong>{t('credits.deficit')}: </strong><SaudiRiyalIcon amount={conflictData.deficit} /></p>
            </div>

            <div className="space-y-2">
              <h6 className="font-semibold text-text-primary text-sm">{t('credits.allocationAdjustments')}</h6>
                  ...
                  <div key={allocation.id} className="rounded-lg border border-border bg-card p-3">
                    <div className="flex justify-between items-start gap-2">
                      <div className="flex-1">
                        <strong className="text-sm text-text-primary block">{allocation.description ?? t('common.noDescription')}</strong>
            ...
            <div className="text-sm font-semibold text-text-primary pt-2 border-t border-status-warning-border">
