...
import { useGetInvoiceAgingAnalysis } from '@/features/financials/api/financialCenterQueries';
...
import InvoicesKanban from '../components/InvoicesKanban';
import {
  X,
  Plus,
  FileText,
...
import InvoiceCancelModal from '../modals/InvoiceCancelModal';
      ...
      <div className="flex flex-wrap justify-between items-center gap-4 mb-4 flex-shrink-0">
        ...
        <div className="bg-white/80 backdrop-blur px-4 py-2 rounded-full border shadow-sm flex items-center gap-4 text-xs overflow-x-auto max-w-full">
          ...
          <div className="flex items-center gap-1 whitespace-nowrap">
            ...
            <span className="text-text-muted">({isAgingLoading ? '-' : agingData?.['90_plus']?.count || 0})</span>
          </div>
        </div>
                ...
                <Table className="border-collapse border border-border-strong">
                  <TableHeader className="bg-muted/50 sticky top-0 z-10 shadow-sm">
                    <TableRow>
                      ...
                      <TableHead className="text-start border border-border-strong px-3 py-2">المهمة/الوصف</TableHead>
                      ...
                      <TableHead className="text-start text-xs text-text-muted border border-border-strong px-3 py-2">الإجمالي</TableHead>
                  ...
                  <TableBody>
                    {allInvoices.map((invoice) => (
                      <TableRow key={invoice.id} className="hover:bg-muted/20 group">
                        ...
                        <TableCell className="border border-border-strong px-3 py-2">
                          <span className="text-sm text-text-secondary block max-w-[200px] truncate" title={invoice.task?.task_name || invoice.description}>
                        ...
                        <TableCell className="border border-border-strong px-3 py-2">
                          <span className={`text-sm ${Number(invoice.remaining_amount) > 0 ? 'text-orange-600 font-medium' : 'text-text-muted'}`}>
                        ...
                        <TableCell className="border border-border-strong px-3 py-2">
                          <div className="flex justify-between gap-1 p-1 ">
                            ...
                            >
                              <Eye size={14} className="text-status-info-text" />
                            </Button>
                            <Button
                              variant="outline-secondary"
                              size="sm"
