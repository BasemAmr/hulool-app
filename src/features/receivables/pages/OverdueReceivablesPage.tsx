...
import { useGetFilteredReceivablesInfinite } from '@/features/receivables/api/receivableQueries';
import { applyPageBackground } from '@/shared/utils/backgroundUtils';
import FilteredReceivablesTable from '../components/FilteredReceivablesTable';
  ...
  if (!hasViewOverdueReceivablesPermission) {
    return (
      <div className="rounded-lg border border-yellow-500 bg-status-warning-bg0/10 p-4 text-center">
        ...
        <p className="text-text-primary">You don't have permission to view overdue receivables.</p>
      </div>
    );
  }
        ...
        <div className="p-0">
          ...
          <div ref={ref} className="text-center p-4">
            ...
            {!hasNextPage && !isLoading && allReceivables.length > 0 && (
              <p className="text-text-primary mb-0">وصلت إلى نهاية القائمة</p>
            )}
          </div>
        </div>
      </div>
