...
export interface HuloolGridProps<T extends Record<string, any>> {
  ...
  rowHeight?: number;
  /** Field used for type-based row alternating */
  typeField?: keyof T;
...
// ================================
// TYPE-BASED ROW COLORS — all rows use calm semantic surface tokens
// ================================
...
const DueCell = React.memo(({ rowData, columnData, active }: CellProps<any, { key: string }>) => {
  ...
  const numValue = Number(value) || 0;
  return (
    <span className="hulool-cell-content hulool-due" style={{ color: numValue > 0 ? 'var(--token-text-danger)' : 'var(--token-text-primary)', fontWeight: active ? 800 : 600 }}>
...
const BadgeCell = React.memo(({ rowData, columnData, active }: CellProps<any, { key: string; badgeColors?: Record<string, string>; formatter?: (val: any, row: any) => string }>) => {
  ...
  const defaultColors: Record<string, string> = {
    Government: 'var(--token-status-info-bg)', RealEstate: 'var(--token-status-success-bg)',
    Accounting: 'var(--token-status-warning-bg)', Other: 'var(--token-status-neutral-bg)',
    New: 'var(--token-status-warning-bg)', Deferred: 'var(--token-status-neutral-bg)',
    Completed: 'var(--token-status-success-bg)', 'Pending Review': 'var(--token-status-info-bg)',
    Cancelled: 'var(--token-status-neutral-bg)',
  };

  const bgColor = { ...defaultColors, ...columnData?.badgeColors }[value] || 'var(--token-status-neutral-bg)';

  return (
    <span className="hulool-cell-content hulool-badge" style={{ backgroundColor: bgColor, color: 'var(--token-text-primary)', fontWeight: active ? 700 : 600 }}>
      {displayValue ?? value ?? '—'}
    </span>
  );
});
  ...
  return (
    ...
    >
      <button
        ...
        onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'var(--token-bg-surface-hover)'}
    ...
    <div className={`hulool-data-grid ${className}`}>
      ...
      <style>{`
        ...
        .hulool-spinner {
          width: 40px;
          height: 40px;
          border: 4px solid var(--token-action-primary-bg);
        ...
        .hulool-data-grid .dsg-container {
          --dsg-border-color: var(--token-border-default);
        ...
        .hulool-data-grid .dsg-active-cell {
          box-shadow: inset 0 0 0 2px var(--token-border-focus) !important;
        }
        ...
        .hulool-debit { color: var(--token-text-primary); }
        .hulool-credit { color: var(--token-text-success); }
        ...
        /* ================================
           ROW COLORS - SIMPLE ALTERNATING (all types use the same calm tokens)
           ================================ */
        
        .hulool-data-grid .dsg-row.hulool-row-even .dsg-cell,
        .hulool-data-grid .dsg-row.hulool-type-base .dsg-cell {
          background-color: ${DEFAULT_ALTERNATE_COLORS.even};
        }
        
        .hulool-data-grid .dsg-row.hulool-row-odd .dsg-cell,
        .hulool-data-grid .dsg-row.hulool-type-alternate .dsg-cell {
