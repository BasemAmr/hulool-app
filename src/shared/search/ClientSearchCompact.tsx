...
import { useSearchClients, useCreateClient } from '@/features/clients/api/clientQueries';
  ...
  return (
    <div className="client-search-compact">
      ...
      {!isLoading && debouncedSearchTerm && clients.length > 0 && (
        <div className="shadow-sm rounded-lg overflow-hidden mb-2 border border-border-default">
          {clients.map(client => (
            <button
              key={client.id}
              type="button"
      ...
      {!isLoading && debouncedSearchTerm && clients.length === 0 && (
        <div className="mb-2">
          <div className="bg-status-info-bg border border-status-info-border text-blue-800 py-2 px-3 rounded-lg mb-2 text-sm">
